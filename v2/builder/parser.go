...
func ParseFile(fname string) ([]Vec, [][3]int, error) {
	f, err := os.Open(fname)
	if err != nil {
		return nil, nil, err
	}
...
func Parse(r io.Reader) (vertices []Vec, faces [][3]int, e error) {
	defer func() {
		if p := recover(); p != nil {
