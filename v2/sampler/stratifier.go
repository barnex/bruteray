...
func NewStratifier(f tracer.ImageFunc, w, h int, antiAlias bool) *Stratifier {
	return &Stratifier{
		f:         f,
...
func (s *Stratifier) Sample(nPass int) {
...
func (s *Stratifier) Stats() string {
	t := s.WallTime.Seconds()
	return fmt.Sprintf("%.1f rays/s, %.1f pix/s", float64(s.RayCount)/t, float64(s.PixelCount)/t)
}

func (s *Stratifier) SampleLine(ctx *tracer.Ctx, iy, w, h, budget int, totalVariance float64) {
	for ix := 0; ix < w; ix++ {
		n := 1
		if s.totalPasses > 30 {
...
func (s *Stratifier) samplePixel(ctx *tracer.Ctx, ix, iy, w, h int, n int) {
	ctx.PixelCount++
	xi, yi := IndexToCam(ix, iy, w, h)
...
func (s *Stratifier) filteredVariance(i, j int) float64 {
...
func (s *Stratifier) varianceGamma(i, j int) float64 {
...
func (s *Stratifier) variance3(i, j int) colorf.Color {
	sum := s.sum[j][i]
	sumSq := s.sumSq[j][i]
	n := s.n[j][i]
...
func (s *Stratifier) stdErr(i, j int) float64 {
	v := s.variance3(i, j)
	return math.Sqrt(float64((v.R + v.G + v.B) / s.n[j][i]))
}

func (s *Stratifier) TotalVariance() float64 {
	total := 0.
	for iy := range s.sum {
...
func (s *Stratifier) Image() image.Image {
	return stratifiedImage{s}
}

func (s *Stratifier) StdDevImage() image.Image { return stratifiedStdDev{s} }
func (s *Stratifier) StdErrImage() image.Image { return stratifiedStdErr{s} }

func (s *Stratifier) SamplingImage() image.Image {
	img := s.n
	var max float64
	for iy := range img {
		for ix := range img[iy] {
...
func (s *Stratifier) Bounds() image.Rectangle {
	return image.Rect(0, 0, len(s.sum[0]), len(s.sum))
}

// ColorModel implements image.Image
func (s *Stratifier) ColorModel() color.Model {
	return nil
}

type stratifiedImage struct{ *Stratifier }
type stratifiedStdDev struct{ *Stratifier }
type stratifiedStdErr struct{ *Stratifier }
...
func (s *Stratifier) Color64At(i, j int) colorf.Color {
	return s.sum[j][i].Mul(float64(1 / s.n[j][i]))
}

func (s *Stratifier) StoredImage() imagef.Image {
	w, h := s.Bounds().Dx(), s.Bounds().Dy()
...
func IndexToCam(ix, iy, w, h int) (camx, camy float64) {
